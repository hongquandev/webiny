 ...
 */
export default class TodosQueryImplementation extends TodosResolver implements TodosQuery {
    /**
    ...
    async listTodos({ limit = 10, sort, after, before }: ListTodosParams) {
        const PK = this.getPK();
        const query: TodosQueryParams = {
            limit,
            reverse: sort !== "createdOn_ASC",
            gt: undefined,
            lt: undefined
        };
        const meta: TodosMetaParams = { limit, after: null, before: null };
